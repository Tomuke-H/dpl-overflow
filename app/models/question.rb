class Question < ApplicationRecord
  ...
  def self.find_questions_by_tag(tag_name)
    select('DISTINCT q.id, q.views, q.likes, count(t.id) AS tags, count(a.id) AS total_answers, q.title, q.body, q.created_at AS q_created')
    .from('questions AS q')
...
def self.unanswered_questions
    select('DISTINCT q.id, q.views, q.likes, q.title, q.body, count(a.id) AS total_answers')
    .from('questions AS q') 
