  ...
  const helpCode =() => {
    return(
      <Modal show={showHelp} onHide={()=>setShowHelp(!showHelp)}>
        ...
        <Modal.Body>
        2.  If there is only one line of code, a set of single backticks will suffice.  However, it is recommended that you use triple backticks, especially if your code has any backticks in its content.  Otherwise, the backticks in your code content will need to be wrapped in a set of curly brackets and quotes.  In other words, you would need to type {"{'`'}"} instead of just {'`'}.
        </Modal.Body>
        <Modal.Body>
        ...
        <Modal.Body>
        4. When using triple backticks to show your code, you can add a language after the initial set of backticks to display the code differently (for example, {'```'} ruby). Otherwise, the styling on your codeblock will be optimized for JavaScript.
        </Modal.Body>
        <Modal.Footer>
